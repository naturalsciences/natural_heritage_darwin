...
class MaCollectorForm extends BaseForm
{
   public function configure()
  {
  
		 $this->widgetSchema->setNameFormat('mass_action[MassActionForm][collectors][%s]');
		 $subForm = new sfForm();
...
public function bind(array $taintedValues = null, array $taintedFiles = null)
  ...
  public function doMassAction($user_id, $items, $values)
  {
		...
		{
			$old_data=Doctrine_Core::getTable('CataloguePeople')->getPeopleRelated('specimens','collector',  $result->getId());
			...
			foreach($list_people  as $p_id)
			{
				if($p_id!==null)
				{
					...
					$cataloguepeople->setPeopleType("collector");
