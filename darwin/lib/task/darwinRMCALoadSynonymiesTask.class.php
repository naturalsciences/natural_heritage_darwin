...
class darwinRMCALoadSynonymiesTask extends sfBaseTask
{
  ...
  protected function configure()
  {
    $this->addOptions(array(
      ...
      new sfCommandOption('id', null, sfCommandOption::PARAMETER_REQUIRED, 'Only do the job for a given import id')));
		...
		$this->name             = 'import-synonyms';
		$this->briefDescription = 'Attach synonyms to taxon';
	   ...
	   if(!empty($options['id']) && ctype_digit($options['id']) )
	  {
		  ...
		  try
		  {
				...
				if($import_obj!==null)
				{
					...
					{
						$tabParser = new RMCATabImportSynonymies( $id_import);
