...
class darwinDoImportCodesTask extends sfBaseTask
{
 ...
 protected function execute($arguments = array(), $options = array())
  {
	...
	{	
		...
		if($import->getFormat()=="codes")
		{
			$this->log("Going for ID ".$import_id);
			$conn->beginTransaction();
			try 
			{
				$conn->execute("SELECT darwin2.fct_rmca_imp_manager_create_code(staging_synonymies.*)
    ...
    $this->log("End Check Codes : ".date('G:i:s'));
  }
}