...
class darwinDoImportRelationshipsTask extends sfBaseTask
{
	protected function configure()
  {
    ...
    $this->name             = 'do-import-relationships';
    $this->briefDescription = 'Store checked relationships into the database ';
 ...
 protected function execute($arguments = array(), $options = array())
  {
	...
	{	
		...
		if($import->getFormat()=="relationships")
		{
			$this->log("Going for ID ".$import_id);
			$conn->beginTransaction();
			try 
			{
				$conn->execute("SELECT darwin2.fct_rmca_imp_manager_create_relationship(staging_update_specimen_relationship.*)
					FROM staging_update_specimen_relationship where import_ref=?;",[$import_id]);
				//print("SELECT darwin2.fct_rmca_imp_manager_create_property(staging_properties.*)
				//	FROM staging_properties where import_ref=");
				$sql_prepared = $conn->prepare("UPDATE imports set state='finished', is_finished = TRUE WHERE id = ?");
				$sql_prepared->execute([$import_id]);
				$conn->commit();
				
				$conn->beginTransaction();
				$conn->execute("select fct_rmca_import_check_relationship_launch(?)",[$import_id]);				
				$sql_prepared = $conn->prepare("UPDATE imports set state='finished', is_finished = TRUE WHERE id = ?");
				$sql_prepared->execute([$import_id]);
				$conn->commit();
			}			
    ...
    $this->log("End Check Prop : ".date('G:i:s'));
  }
}