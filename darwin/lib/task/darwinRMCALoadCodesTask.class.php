...
class darwinRMCALoadCodesTask extends sfBaseTask
{
  ...
  protected function configure()
  {
    $this->addOptions(array(
      ...
      new sfCommandOption('id', null, sfCommandOption::PARAMETER_REQUIRED, 'Only do the job for a given import id')));
		...
		$this->name             = 'import-codes';
		$this->briefDescription = 'Attach or replace codes to specumebs';
	   ...
	   if(!empty($options['id']) && ctype_digit($options['id']) )
	  {
		  ...
		  try
		  {
				...
				if($import_obj!==null)
				{
					...
					{
							//print("C");
						$tabParser = new RMCATabImportCodes( $id_import);
