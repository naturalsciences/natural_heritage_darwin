...
/*ftheeten 2016 06 22*/
 ...
 /*ftheeten 2016 07 05*/
   ...
   //ftheeten 2017 01 12
    ...
    //2019 03 29
	$this->widgetSchema['property_fuzzy'] = new sfWidgetFormInputCheckbox(array('default' => TRUE));
  	////ftheeten 2015 09 09
    ...
    $this->widgetSchema['loan_is_closed'] = new sfWidgetFormChoice(array(
        'choices'  => array("any"=> "Any specimen", "all_loans"=> "All loans" , "no_loans"=> "No loans", "closed" => 'closed', "ongoing" => 'ongoing', "gift" => 'gift'),
       'expanded'=> true
        ), array( 'style' => "display: inline-block;text-align:center"));
    //ftheeten 2017 04 27
  ...
  public function addCommentsQuery($query, $notion, $comment)
  {
    ...
    if($notion != '' || $comment != '') {
      ...
      if($notion != '')
	  {
		...
		$sql_member[]='notion_concerned = ?';
		$sql_params[]=$notion;
      }
	  if($comment != '')
	  {
        //$query->andWhere('comment_indexed like concat(\'%\', fulltoindex(?), \'%\' )', $comment);
		$sql_member[]="comment_indexed like '%'||fulltoindex(?)||'%'";
		$sql_params[]=$comment;
	  }
      $this->with_group = true;
	  if(count($sql_member)>0)
	  {
		  $query->andWhere("(EXISTS(SELECT 1 FROM Comments   WHERE record_id=s.id AND referenced_relation='specimens' AND ".implode(" AND ", $sql_member)."))",$sql_params);
	  }
    }
    return $query ;
  }
...
/*pvignaux20160606*/
 ...
 public function addSpecimenCountJuvenilesMaxColumnQuery($query, $field, $val)
 {
  ...
  public function addEcologyQuery($query, $val)
  {
    if($val != '') 
	{
      $query->andWhere("EXISTS(SELECT 1 FROM Comments  WHERE record_id=s.id AND referenced_relation='specimens' AND notion_concerned='ecology' AND comment_indexed LIKE '%'||fulltoindex(?)||'%' )",$val);
    }
    return $query ;
  }
  ...
  {
    if($val != '') {
      $query->andWhere("EXISTS (SELECT s.id FROM LoanItems WHERE LoanItems.specimen_ref=s.id)");
  ...
  public function addInClosedLoansQuery($query, $val)
  {
    ...
    if($val != '') {		
		...
		elseif($val=="no_loans")
        {
           ...
           $query->andWhere("NOT EXISTS (SELECT s.id FROM LoanItems WHERE LoanItems.specimen_ref=s.id)");
        }
		elseif($val=="closed")
        {

           $query->andWhere("EXISTS (SELECT s.id FROM LoanItems WHERE LoanItems.specimen_ref=s.id AND LoanItems.loan_ref = ANY (fct_rmca_return_loans_by_status(?)))", "closed");
        }
		elseif($val=="ongoing")
        {

           $query->andWhere("EXISTS (SELECT s.id FROM LoanItems WHERE LoanItems.specimen_ref=s.id AND LoanItems.loan_ref = ANY (fct_rmca_return_loans_not_in_status(?)))", "returned");
        }
        elseif($val=="gift")
        {

           $query->andWhere("EXISTS (SELECT s.id FROM LoanItems l WHERE l.specimen_ref=s.id 
           AND EXISTS(SELECT p.id FROM Properties p WHERE p.referenced_relation='loans' AND p.property_type='gift' AND l.loan_ref= p.record_id) )");
        }
            ...
            if(strlen($tagvalue)>0)
            {
				...
				else
				{
					$tmpStr[]="(
							...
							EXISTS(SELECT s.id FROM Tags t where lower(tag) ~ ('(^|\s+)'||LOWER($tagvalue)||'($|\s+)') and t.gtu_ref=s.gtu_ref) 
						  )";
				}
