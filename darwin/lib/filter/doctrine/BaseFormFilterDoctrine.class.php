...
abstract class BaseFormFilterDoctrine extends sfFormFilterDoctrine
{
  ...
  */
				...
				if(is_numeric($item_ref))
				{
					...
					if(in_array('synonym',$relations))
					{
						...
						else
						{
							...
							$queryTmp.=$field_prefix."_ref IN (select fct_rmca_taxo_get_syno_no_children(".$item_ref.") )";
						}
					}
  ...
  //ftheeten 2023 05 12
   public static function getCollectionWithAdminRights($user, $db_user_type=4)
  {
      if($user->isA(Users::ADMIN))
      {
        $res = array(0=>0);
      ...
      $conn = Doctrine_Manager::connection();
      $sql = "SELECT collection_ref from collections_rights where user_ref = :userid ";
      $sql .= " and db_user_type >= :db_user_type";

      $q = $conn->prepare($sql);
      $q->execute(array(':userid' => $user->getId(),  ':db_user_type'=> $db_user_type));
      $colls = $q->fetchAll();
      $results = array(0=>0);
      foreach($colls as $col)
      {
