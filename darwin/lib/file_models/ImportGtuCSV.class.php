...
class ImportGtuCSV
{
    ...
    private function initFields()
    {
		...
		$fields[40]="sampling_method";
		$fields[41]="sampling_fixation";
		$fields[42]="station_notes";
		$fields[43]="sampling_notes"; 
        $fields[44]="bounding_box"; 
		$fields[45]="samplingcode";
	...
	//print("____________PARSE_AND_SAVE");
		...
		{
			//print("RETEST");
			$samplingCode=$this->getValueIfFieldExists("samplingcode", $p_row);
		}
        if($samplingCode)
        {
			//print("exists");
		...
		else
		{
				...
				if($this->conn->getDbh()->inTransaction())
				{
				 ...
				 $ne=new Doctrine_Exception(  $errors_reported );
