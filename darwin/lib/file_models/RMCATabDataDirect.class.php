...
class RMCATabDataDirect
{
    ...
    private function initFields()
    {
        ...
        $fields[] = "associationDomain"; /* values : darwin_full_code, darwin_id, darwin_uuid, darwin_file, taxonomy, mineralogy, external */
  ...
  function align_quote($p_text, $p_encoding="UTF-8")
  {
	 ...
	 private function saveUnit()
	{
		...
		catch(Doctrine_Exception $ne)
		{
			...
			$ok = false ;
			$this->import->setErrorsInImport("Table error for staging".$this->errors_reported);
			//$this->import->save();
			print("IMPORT MODIFIED");*/
			$ok = false ;
			$import_obj = Doctrine_Core::getTable('Imports')->find($this->import_id);			
			$import_obj->setErrorsInImport($ne->getMessage());
			$import_obj->setState("error");
...
//print("try2");		  
    ...
    foreach($this->multimedia as $object)
    {
      ...
      catch(Doctrine_Exception $ne)
      {
        //$e = new DarwinPgErrorParser($ne);
        /*
		$this->errors_reported .= "Multimedia ".$object->getUri()." was not saved : ".$e->getMessage().";";
		 $e = new DarwinPgErrorParser($ne);
