...
Dim specimenID, additionalID, datasetName, accessionNumber, isPhysical, acquisitionType, acquiredFrom, acquisitionDay, acquisitionMonth, acquisitionYear
Dim phylum, classis, ordo, superfamilia, familia, subfamilia, genus, subgenus, oldGenus, oldSubgenus, species, author_year, subspecies, variety_form, taxonFullName, informalName
Dim identifiedBy, identificationDay, identificationMonth, identificationYear, identificationNotes, referenceString, publicationString, urlPicture
...
Dim populatedPlace, naturalSite, exactSite, samplingCode, expedition_project, elevationInMeters, depthInMeters, latitude, longitude
Dim ecology, samplingMethod, collectedBy, collectionStartDay, collectionStartMonth, collectionStartYear, collectionStartTimeH, collectionStartTimeM
Dim collectionEndDay, collectionEndMonth, collectionEndYear, collectionEndTimeH, collectionEndTimeM, localityNotes
Dim kindOfUnit, associatedUnitInstitution, associatedUnitCollection, associatedUnitID, associationType
Dim sex, lifeStage, statusType, totalNumber, adultCount, larvaCount, pupaCount, juvenileCount, immatureCount, stageUnknownCount, maleCount, femaleCount
Dim sexUnknownCount, deutonymphCount, protonymphCount, tritonymphCount, socialStatus, fixation, conservation, hostName, creationDay, creationMonth, creationYear
Dim institutionStorage, buildingStorage, floorStorage, roomStorage, laneStorage, columnStorage, shelfStorage
Dim container, containerType, containerStorage, subcontainer, subcontainerType, subcontainerStorage, notes

'Dim boxStorage, tubeStorage, notes
...
If CheckHeaders(check:=False) Then
    ...
    If CopySheetsForRework Then
        ...
        Set node = dom.createComment("Schema ABCD - Template général d'encodage pour les données zoologiques, version définitve 01/2014)")
        dom.appendChild node
        Set node = Nothing
...
'        End If
    ...
    strTestFill = strLocalityCode & strDateStart & strDateStartT & strDateEnd & strDateEndT & strGatheringAgent & strProject & strMethod & strLocalityText & rep _
    & strElevation & strDepth & strBiotope & strGatheringNotes
...
'    On Error GoTo Err_XMLMeasurements
    ...
    Dim strJuvenileCount As String
    Dim strImmatureCount As String
    Dim strStageUnknownCount As String
    Dim strDeutonymphCount As String
    Dim strProtonymphCount As String
    Dim strTritonymphCount As String
    Dim strSocialStatus As String
    Dim strHost As String
    Dim strCreationDD As String, strCreationMM As String, strCreationYY As String, strCreationDate As String
    Dim strTestFill As String
    ...
    strJuvenileCount = Application.Sheets("cSPECIMEN").Cells(rowCounter, Application.Sheets("cSPECIMEN").Rows(1).Find(juvenileCount, lookAt:=xlWhole).Column).Value
    strImmatureCount = Application.Sheets("cSPECIMEN").Cells(rowCounter, Application.Sheets("cSPECIMEN").Rows(1).Find(immatureCount, lookAt:=xlWhole).Column).Value
    strStageUnknownCount = Application.Sheets("cSPECIMEN").Cells(rowCounter, Application.Sheets("cSPECIMEN").Rows(1).Find(stageUnknownCount, lookAt:=xlWhole).Column).Value
    strProtonymphCount = Application.Sheets("cSPECIMEN").Cells(rowCounter, Application.Sheets("cSPECIMEN").Rows(1).Find(protonymphCount, lookAt:=xlWhole).Column).Value
    strDeutonymphCount = Application.Sheets("cSPECIMEN").Cells(rowCounter, Application.Sheets("cSPECIMEN").Rows(1).Find(deutonymphCount, lookAt:=xlWhole).Column).Value
    strTritonymphCount = Application.Sheets("cSPECIMEN").Cells(rowCounter, Application.Sheets("cSPECIMEN").Rows(1).Find(tritonymphCount, lookAt:=xlWhole).Column).Value
    strHost = Application.Sheets("cSPECIMEN").Cells(rowCounter, Application.Sheets("cSPECIMEN").Rows(1).Find(hostName, lookAt:=xlWhole).Column).Value
     
    strCreationDD = Application.Sheets("cSPECIMEN").Cells(rowCounter, Application.Sheets("cSPECIMEN").Rows(1).Find(creationDay, lookAt:=xlWhole).Column).Value
    If strCreationDD <> "" And IsNumeric(strCreationDD) Then
        If strCreationDD > 31 Or strCreationDD = 0 Then
            strCreationDD = ""
        ElseIf strCreationDD < 10 Then
            strCreationDD = "0" & strCreationDD
        End If
    Else:
        strCreationDD = ""
    End If
    strCreationMM = Application.Sheets("cSPECIMEN").Cells(rowCounter, Application.Sheets("cSPECIMEN").Rows(1).Find(creationMonth, lookAt:=xlWhole).Column).Value
    If strCreationMM <> "" And IsNumeric(strCreationMM) Then
        If strCreationMM > 12 Or strCreationMM = 0 Then
            strCreationMM = ""
        ElseIf strCreationMM < 10 Then
            strCreationMM = "0" & strCreationMM
        End If
    Else:
        strCreationMM = ""
    End If
    strCreationYY = Application.Sheets("cSPECIMEN").Cells(rowCounter, Application.Sheets("cSPECIMEN").Rows(1).Find(creationYear, lookAt:=xlWhole).Column).Value
    If strCreationYY <> "" And IsNumeric(strCreationYY) Then
        If strCreationYY > 999 Then
            strCreationYY = strCreationYY
        Else:
            strCreationYY = ""
        End If
    Else:
    ...
    If strCreationYY <> "" And strCreationMM <> "" And strCreationDD <> "" Then
        strCreationDate = strCreationYY & "-" & strCreationMM & "-" & strCreationDD
    ElseIf strCreationYY <> "" And strCreationMM <> "" And strCreationDD = "" Then
        strCreationDate = strCreationYY & "-" & strCreationMM
    ElseIf strCreationYY <> "" And strCreationMM = "" And strCreationDD = "" Then
        strCreationDate = strCreationYY
    ElseIf strCreationYY = "" And strCreationMM = "" And strCreationDD = "" Then
        strCreationDate = ""
    End If
     
    strTestFill = strTotalCount & strMaleCount & strFemaleCount & strSexUnknownCount & strAdultCount & strLarvaCount & strPupaCount & strJuvenileCount & strImmatureCount & _
    strStageUnknownCount & strProtonymphCount & strDeutonymphCount & strTritonymphCount & strSocialStatus & strHost & strCreationDate
    ...
    If Not IsEmpty(strTestFill) And Not IsNull(strTestFill) And strTestFill <> "" Then
        ...
        If Not IsEmpty(strJuvenileCount) And Not IsNull(strJuvenileCount) And strJuvenileCount <> "" Then
            ...
            Set xmlMeasurementOrFactParameter = dom.createNode(NODE_ELEMENT, "Parameter", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactParameter.Text = "N juveniles"
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactParameter
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
    
            Set xmlMeasurementOrFactLowerValue = dom.createNode(NODE_ELEMENT, "LowerValue", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactLowerValue.Text = strJuvenileCount
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactLowerValue
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
        
        End If

        If Not IsEmpty(strImmatureCount) And Not IsNull(strImmatureCount) And strImmatureCount <> "" Then
            ...
            Set xmlMeasurementOrFactParameter = dom.createNode(NODE_ELEMENT, "Parameter", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactParameter.Text = "N immatures"
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactParameter
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
    
            Set xmlMeasurementOrFactLowerValue = dom.createNode(NODE_ELEMENT, "LowerValue", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactLowerValue.Text = strImmatureCount
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactLowerValue
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
        
        End If

        If Not IsEmpty(strStageUnknownCount) And Not IsNull(strStageUnknownCount) And strStageUnknownCount <> "" Then
            ...
            Set xmlMeasurementOrFactParameter = dom.createNode(NODE_ELEMENT, "Parameter", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactParameter.Text = "N stage unknown"
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactParameter
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
    
            Set xmlMeasurementOrFactLowerValue = dom.createNode(NODE_ELEMENT, "LowerValue", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactLowerValue.Text = strStageUnknownCount
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactLowerValue
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
        
        End If

        If Not IsEmpty(strProtonymphCount) And Not IsNull(strProtonymphCount) And strProtonymphCount <> "" Then
            ...
            Set xmlMeasurementOrFactParameter = dom.createNode(NODE_ELEMENT, "Parameter", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactParameter.Text = "N protonymphs"
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactParameter
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
    
            Set xmlMeasurementOrFactLowerValue = dom.createNode(NODE_ELEMENT, "LowerValue", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactLowerValue.Text = strProtonymphCount
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactLowerValue
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
        
        End If

        If Not IsEmpty(strDeutonymphCount) And Not IsNull(strDeutonymphCount) And strDeutonymphCount <> "" Then
            ...
            Set xmlMeasurementOrFactParameter = dom.createNode(NODE_ELEMENT, "Parameter", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactParameter.Text = "N deutonymphs"
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactParameter
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
    
            Set xmlMeasurementOrFactLowerValue = dom.createNode(NODE_ELEMENT, "LowerValue", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactLowerValue.Text = strDeutonymphCount
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactLowerValue
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
        
        End If
    
        If Not IsEmpty(strTritonymphCount) And Not IsNull(strTritonymphCount) And strTritonymphCount <> "" Then
            ...
            Set xmlMeasurementOrFactParameter = dom.createNode(NODE_ELEMENT, "Parameter", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactParameter.Text = "N tritonymphs"
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactParameter
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
    
            Set xmlMeasurementOrFactLowerValue = dom.createNode(NODE_ELEMENT, "LowerValue", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactLowerValue.Text = strTritonymphCount
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactLowerValue
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
        
        End If

        If Not IsEmpty(strHost) And Not IsNull(strHost) And strHost <> "" Then
            ...
            Set xmlMeasurementOrFactParameter = dom.createNode(NODE_ELEMENT, "Parameter", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactParameter.Text = "Host"
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactParameter
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
    
            Set xmlMeasurementOrFactLowerValue = dom.createNode(NODE_ELEMENT, "LowerValue", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactLowerValue.Text = strHost
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactLowerValue
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
        
        End If
        
        If Not IsEmpty(strCreationDate) And Not IsNull(strCreationDate) And strCreationDate <> "" Then
            ...
            Set xmlMeasurementOrFactParameter = dom.createNode(NODE_ELEMENT, "Parameter", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactParameter.Text = "Creation date"
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactParameter
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
    
            Set xmlMeasurementOrFactLowerValue = dom.createNode(NODE_ELEMENT, "LowerValue", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactLowerValue.Text = strCreationDate
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactLowerValue
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
...
stageUnknownCount = "stageUnknownCount"
maleCount = "maleCount"
femaleCount = "femaleCount"
sexUnknownCount = "sexUnknownCount"
deutonymphCount = "deutonymphCount"
protonymphCount = "protonymphCount"
tritonymphCount = "tritonymphCount"
...
SpecHeadings = Array("specimenID", "additionalID", "datasetName", "accessionNumber", "isPhysical", "acquisitionType", "acquiredFrom", "acquisitionDay", _
"acquisitionMonth", "acquisitionYear", "phylum", "classis", "ordo", "superfamilia", "familia", "subfamilia", "genus", "subgenus", "species", "author_year", _
"subspecies", "variety_form", "taxonFullName", "informalName", "oldGenus", "oldSubgenus", "identifiedBy", "identificationDay", "identificationMonth", "identificationYear", _
"identificationNotes", "referenceString", "publicationString", "urlPicture", "samplingCode", "expedition_project", "ocean", "continent", "sea", "country", _
"state_territory", "province", "region", "archipelago", "district", "county", "department", "island", "city", "municipality", "populatedPlace", "naturalSite", _
"exactSite", "elevationInMeters", "depthInMeters", "latitude", "longitude", "ecology", "samplingMethod", "collectedBy", _
"collectionStartDay", "collectionStartMonth", "collectionStartYear", "collectionStartTimeH", "collectionStartTimeM", "collectionEndDay", _
"collectionEndMonth", "collectionEndYear", "collectionEndTimeH", "collectionEndTimeM", "localityNotes", "kindOfUnit", "associatedUnitInstitution", _
"associatedUnitCollection", "associatedUnitID", "associationType", "sex", "lifeStage", "statusType", "totalNumber", "adultCount", "larvaCount", _
"pupaCount", "juvenileCount", "immatureCount", "stageUnknownCount", "maleCount", "femaleCount", "sexUnknownCount", "deutonymphCount", _
"protonymphCount", "tritonymphCount", "socialStatus", "fixation", "conservation", "hostName", "creationDay", "creationMonth", "creationYear", _
"institutionStorage", "buildingStorage", "floorStorage", "roomStorage", "laneStorage", "columnStorage", "shelfStorage", _
