...
class catalogueActions extends DarwinActions
{
  protected $catalogue = array(
   ...
   'people_languages', 'people_relationships', 'classification_keywords','catalogue_bibliography', 'multimedia', 'identifiers');
  ...
  public function executeCompleteName(sfWebRequest $request) {
    ...
    if(in_array($tbl,$catalogues)) {
        ...
        {
            $result =Doctrine_Core::getTable("taxonomy")->getTaxonByNameAndCollectionAndLevel($request->getParameter('term'), $request->getParameter('level'), $request->getParameter('collections'),true);
        }
        //ftheeten 2017 06 26
        elseif($tbl=="taxonomy" && $request->getParameter('collections') && !$request->getParameter('level'))
        {
            $result =Doctrine_Core::getTable("taxonomy")->getTaxonByNameAndCollection($request->getParameter('term') , $request->getParameter('collections'),true);
        }
        //ftheeten 2017 06 26
        elseif($tbl=="taxonomy" && $request->getParameter('level') && ! $request->getParameter('collections'))
        {
              $result =Doctrine_Core::getTable("taxonomy")->getTaxonByNameAndLevel($request->getParameter('term') , $request->getParameter('level'),true);
        }
        else
        {
          ...
          {
            $result = Doctrine_Core::getTable($model)->completeAsArray($this->getUser(), $request->getParameter('term'), $request->getParameter('exact'),30,'',true);
          }
          else
          {
          //ftheeten 2018 10 01 additional parameter for agg (parallel taxonomies)
            $result = Doctrine_Core::getTable($model)->completeWithLevelAsArray($this->getUser(), $request->getParameter('term'), $request->getParameter('exact'),30, $request->getParameter('level'),true);
           }
        }
    }else{
	...
	if($request->getParameter('term'))
	{
		 ...
		 //
			if(strtolower($category)!="all")
			{
				...
				{
					$sql = "SELECT DISTINCT COALESCE(code_prefix,'')||COALESCE(code_prefix_separator,'')||COALESCE(code,'')||COALESCE(code_suffix_separator,'')||COALESCE(code_suffix,'') as value, full_code_indexed as value_indexed,length(code) as len  FROM mv_codes_series WHERE code_category=:category AND referenced_relation='specimens' AND
					serie_indexed LIKE CONCAT('%', (SELECT * FROM fulltoindex(:term)), '%') AND 
				 ...
				 {
					$sql = "SELECT DISTINCT COALESCE(code_prefix,'')||COALESCE(code_prefix_separator,'')||COALESCE(code,'')||COALESCE(code_suffix_separator,'')||COALESCE(code_suffix,'') as value, full_code_indexed as value_indexed,length(code) as len  FROM mv_codes_series WHERE code_category=:category AND
					referenced_relation='specimens' AND 
					serie_indexed LIKE CONCAT('%', (SELECT * FROM fulltoindex(:term)), '%')   ORDER by length(code), full_code_indexed LIMIT 30;";
				}
				$q = $conn->prepare($sql);
			...
			else
			{
				...
				if($request->getParameter('collections'))
				 {
					$sql = "SELECT DISTINCT code_full as value, serie_indexed as value_indexed,length(code_full) as len FROM mv_codes_series WHERE referenced_relation='specimens' AND
					serie_indexed LIKE CONCAT('%', (SELECT * FROM fulltoindex(:term)), '%') AND 
					record_id IN (SELECT id FROM specimens WHERE collection_ref IN (".$request->getParameter('collections').") )
					ORDER by length(code_full), serie_indexed LIMIT 30;";
				 }
				 else
				 {
					$sql = "SELECT DISTINCT code_full as value, serie_indexed as value_indexed,length(code_full) as len  FROM mv_codes_series WHERE 
					referenced_relation='specimens' AND 
					serie_indexed LIKE CONCAT('%', (SELECT * FROM fulltoindex(:term)), '%')   ORDER by length(code_full), serie_indexed LIMIT 30;";
				}
  ...
  public function executeCodesTaxonAutocompleteForLoans(sfWebRequest $request)
  {
	...
	if($request->getParameter('term'))
	{
		 ...
		 if($request->getParameter('collections'))
		 {
            ...
            , full_code_indexed||COALESCE(taxon_name_indexed,'')  as value_indexed , full_code_indexed as sort FROM specimens INNER JOIN
            codes  
            ON
