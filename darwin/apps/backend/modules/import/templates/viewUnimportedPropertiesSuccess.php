...
<form action="<?php print(url_for("import/viewUnimportedProperties"));?>/id/<?php print($id);?>">
Count all : <?php print($size_data);?><br/>
Current page : <?php print($page);?> / <?php print($max_page);?> <br/>
Page : 
<?php if($page>1):?><a href="<?php print(url_for("import/viewUnimportedProperties"));?>?id=<?php print($id);?>&page=<?php print($page-1);?>"><?php print(__("<"));?></a><?php endif;?>
...
<?php if($page<$max_page):?><a href="<?php print(url_for("import/viewUnimportedProperties"));?>?id=<?php print($id);?>&page=<?php print($page+1);?>"><?php print(__(">"));?></a><?php endif;?>
<br/>
...
<th>Count</th>
 ...
 <?php $sum=0; $i=0; foreach($stats as $stat):?>
    <tr <?php ($stat['import_exception']=="imported_property"||$stat['imported'])? print("class='fld_ok'") : print("class='fld_tocomplete'"); ?>>
...
<a href="<?php print(url_for("import/importproperties"));?>?id=<?php print($id);?>"><?php print(__("import"));?></a>
<br/>
<br/>
 ...
 <?php $i=0; foreach($items as $item):?>
    ...
    <tr <?php ( $item['imported'])? print("class='fld_ok'") : print("class='fld_tocomplete'"); ?>>
		<?php $text; $text = $item['import_exception'] ?: "None" ?>
		 ...
		 <td><?php print($item['property_type']);?></td> 
		 <td><?php print($item['applies_to']);?></td> 
		 <td><?php print($item['lower_value']);?></td> 
		 <td><?php print($item['upper_value']);?></td> 
		 <td><?php $item['is_quantitative']?print("true"):print("false");?></td> 
	     <td><?php print($item['property_unit']);?></td> 
		  <td><?php print($item['method']);?></td> 
		  <td><?php print($item['accuracy']);?></td> 
		  <td><?php print($item['date_from']);?></td> 
		  <td><?php print($item['date_to']);?></td> 
				...
				$(".save_specimen_ref").click(
					function()
					{
						...
						if(tmp.length>0)
						{
							...
							console.log("go");
							 $.getJSON( "<?php print(url_for("import/updateStagingProp"));?>", {stag_id: tmp_imp, spec_id:tmp}, function( result ) {
							 console.log(result);
