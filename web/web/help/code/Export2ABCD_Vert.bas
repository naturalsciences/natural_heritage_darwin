...
Dim specimenID, datasetName, code, accessionNumber, acquisitionType, acquiredFrom, acquisitionDay, acquisitionMonth, acquisitionYear
Dim continent, country, state_province, region_district, municipality, exactSite, latitude, longitude, elevationInMeters, samplingCode, collectedBy
Dim expedition_project, collectionStartDay, collectionStartMonth, collectionStartYear, collectionEndDay, collectionEndMonth, collectionEndYear, localityNotes
Dim phylum, classis, ordo, familia, genus, species, author_year, subspecies, taxonFullName, informalName, identificationHistory, identifiedBy, identificationDay, identificationMonth, identificationYear, identificationNotes
Dim totalNumber, sex, lifeStage, kindOfUnit, countByPart, institutionStorage, buildingStorage, floorStorage, roomStorage, laneStorage, columnStorage, shelfStorage
...
If CheckHeaders(check:=False) Then
    ...
    If CopySheetsForRework Then
        ...
        For rowCounter = 2 To LastR
            ...
            If (XMLMeasurements(dom:=dom, subnode:=subnode, rowCounter:=rowCounter) = False) Then
...
'    On Error GoTo Err_XMLSpecimenUnit
    ...
    Dim strTestFill As String
    strTestFill = strAcquisitionDate & strAcquisitionType & strAcquiredFrom & strAccessionNumber
...
'    On Error GoTo Err_XMLGather
    ...
    strTestFill = strLocalityCode & strDateStart & strDateEnd & strGatheringAgent & strLocalityText & rep & strElevation & strGatheringNotes
...
Private Function XMLMeasurements(ByRef dom As MSXML2.DOMDocument60, ByRef subnode As MSXML2.IXMLDOMElement, ByRef rowCounter As Long) As Boolean
...
'    On Error GoTo Err_XMLMeasurements
    ...
    Dim strTotalCount As String, strSex As String, strCountByPart As String
    ...
    strSex = Application.Sheets("cSPECIMEN").Cells(rowCounter, Application.Sheets("cSPECIMEN").Rows(1).Find(sex, lookAt:=xlWhole).Column).Value
    strCountByPart = Application.Sheets("cSPECIMEN").Cells(rowCounter, Application.Sheets("cSPECIMEN").Rows(1).Find(countByPart, lookAt:=xlWhole).Column).Value

    If Not IsEmpty(strTotalCount) And Not IsNull(strTotalCount) And strTotalCount <> "" _
        Or Not IsEmpty(strSex) And Not IsNull(strSex) And strSex <> "" _
        Or Not IsEmpty(strCountByPart) And Not IsNull(strCountByPart) And strCountByPart <> "" Then
        ...
        If Not IsEmpty(strSex) And Not IsNull(strSex) And strSex <> "" Then
            If strSex <> "M" Or strSex = "F" Or strSex = "U" Or strSex = "N" Or strSex = "X" Or strSex <> "m" Or strSex <> "f" Or strSex <> "u" Or strSex <> "n" Or strSex <> "x" Then
                XMLMeasurements = False
            Else:
                XMLMeasurements = True
                Set xmlMeasurementOrFact = dom.createNode(NODE_ELEMENT, "MeasurementOrFact", "http://www.tdwg.org/schemas/abcd/2.06")
                xmlMeasurementsOrFacts.appendChild xmlMeasurementOrFact
                xmlMeasurementsOrFacts.appendChild dom.createTextNode(vbCrLf + Space$(8))
                xmlMeasurementOrFact.appendChild dom.createTextNode(vbCrLf + Space$(10))
                ...
                Set xmlMeasurementOrFactParameter = dom.createNode(NODE_ELEMENT, "Parameter", "http://www.tdwg.org/schemas/abcd/2.06")
                xmlMeasurementOrFactParameter.Text = "Sex"
                xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactParameter
                xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
        
                Set xmlMeasurementOrFactLowerValue = dom.createNode(NODE_ELEMENT, "LowerValue", "http://www.tdwg.org/schemas/abcd/2.06")
                xmlMeasurementOrFactLowerValue.Text = strSex
                xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactLowerValue
                xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
            End If
        ...
        If Not IsEmpty(strCountByPart) And Not IsNull(strCountByPart) And strCountByPart <> "" Then
            ...
            Set xmlMeasurementOrFactParameter = dom.createNode(NODE_ELEMENT, "Parameter", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactParameter.Text = "Count"
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactParameter
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
    
            Set xmlMeasurementOrFactLowerValue = dom.createNode(NODE_ELEMENT, "LowerValue", "http://www.tdwg.org/schemas/abcd/2.06")
            xmlMeasurementOrFactLowerValue.Text = strCountByPart
            xmlMeasurementOrFactAtomised.appendChild xmlMeasurementOrFactLowerValue
            xmlMeasurementOrFactAtomised.appendChild dom.createTextNode(vbCrLf + Space$(12))
...
'    strBox = Application.Sheets("cSPECIMEN").Cells(rowCounter, Sheets("cSPECIMEN").Rows(1).Find(boxStorage, lookAt:=xlWhole).Column)
    ...
    If strInstitution <> "" Or strBuilding <> "" Or strFloor <> "" Or strRoom <> "" Or strRow <> "" Or strColumn <> "" Or strShelf <> "" _
        ...
        Or strSubcontainerStorage <> "" Or strCode <> "" Then
        'Or strBox <> ""
...
SpecHeadings = Array("specimenID", "datasetName", "code", "accessionNumber", "acquisitionType", "acquiredFrom", "acquisitionDay", "acquisitionMonth", "acquisitionYear", _
"continent", "country", "state_province", "region_district", "municipality", "exactSite", "latitude", "longitude", "elevationInMeters", "samplingCode", "collectedBy", _
"expedition_project", "collectionStartDay", "collectionStartMonth", "collectionStartYear", "collectionEndDay", "collectionEndMonth", "collectionEndYear", "localityNotes", _
"phylum", "classis", "ordo", "familia", "genus", "species", "author_year", "subspecies", "taxonFullName", "informalName", "identificationHistory", "identifiedBy", "identificationDay", "identificationMonth", _
"identificationYear", "identificationNotes", "totalNumber", "sex", "lifeStage", "kindOfUnit", "countByPart", "institutionStorage", "buildingStorage", "floorStorage", "roomStorage", _
"laneStorage", "columnStorage", "shelfStorage", "container", "containerType", "containerStorage", "subcontainer", "subcontainerType", "subcontainerStorage", "notes")
