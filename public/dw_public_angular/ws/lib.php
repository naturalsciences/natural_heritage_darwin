...
function json_darwin_get_collections()
{
    ...
    $rows=array();
    $query="SELECT DISTINCT id, name_full_path as name FROM darwin2.mv_collections_full_path_recursive_public ORDER BY name_full_path";
 ...
 function json_darwin_get_sub_collections($id_coll)
{
	 $conn=connect_to_darwin();
	 $rows=array();
    $query="SELECT DISTINCT id, name_full_path as name FROM darwin2.mv_collections_full_path_recursive_public WHERE path||id::varchar||'/' LIKE '%/'||:id_coll||'/%' ORDER BY name_full_path";
    $stmt=$conn->prepare($query);
...
function json_darwin_get_code($pattern, $collection=-1)
{
    ...
    else
    {
        $query="SELECT DISTINCT COALESCE(code_prefix,'')||COALESCE(code_prefix_separator,'')||COALESCE(code,'')||COALESCE(code_suffix_separator,'')||COALESCE(code_suffix,'') as value FROM codes WHERE code_category='main' AND referenced_relation='specimens' AND
			full_code_indexed LIKE CONCAT('%', (SELECT * FROM fulltoindex(:pattern)), '%') AND 
			-1 = :collection and full_code_indexed like '%rmca%'
			ORDER by value LIMIT 30;
            ";
    }
...
function json_darwin_search_specimens()
{
	...
	try
	{
		...
		if(is_numeric($page_size)&&is_numeric($page))
		{
				...
				SELECT 
				 ...
				 id, uuid, code_prefix, code, code_num, code_display, full_code_indexed, taxon_path, taxon_ref, collection_ref, gtu_country_tag_indexed, gtu_country_tag_value, localities_indexed, gtu_others_tag_value, family, taxon_name, sex, collector_ids, donator_ids, gtu_from_date, gtu_from_date_mask, gtu_to_date, gtu_to_date_mask, coll_type, collection_path, specimen_count_min, latitude, longitude, formated_date, code_concat,
			...
			";
			   ...
			   if(strtoupper($bool_images)=="TRUE"||strtoupper($bool_3d)=="TRUE")
			   {
					...
					{
						$query.= " EXISTS (SELECT ext_links.id FROM ext_links WHERE ext_links.record_id=mv_search_public_specimen.id AND ext_links.referenced_relation='specimens' and category='image_link' ) ";
					}
					...
					if(strtoupper($bool_3d)=="TRUE")
					{
						...
						$query.= " EXISTS (SELECT ext_links.id FROM ext_links WHERE ext_links.record_id=mv_search_public_specimen.id AND ext_links.referenced_relation='specimens' and category='html_3d_snippet' )";
					}
					$query.= ")";
			   }
...
function json_darwin_get_specimen( $uuid)
{
      ...
      if((string)$uuid!="-1")
      {
            ...
            $query="
	SELECT * , count(*) OVER() AS full_count FROM darwin2.mv_specimen_public WHERE uuid=:uuid
        ";          
